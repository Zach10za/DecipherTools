...
chrome.runtime.sendMessage({type: "enabled"}, function(response) {
	...
	if (document.readyState === "complete") {
		...
		let currentTreeNodes = 0;
		observeDOM( leftPane ,function(){ 
			const tree = document.getElementById('daTree');
			if (!tree) return;
			let nodes = tree.childNodes.length;
			if (currentTreeNodes === nodes) return;
			currentTreeNodes = nodes;
			chrome.storage.sync.get(['leftPaneLocked'], (items) => {
				if (items.leftPaneLocked) lockTree();
			});
		});
